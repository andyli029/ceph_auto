...
do
	...
	echo "$ip"
        ...
        sed -i "s/mon initial.*$/mon initial members = node$i/g" ./ceph.conf
        sed -i "s/mon host.*$/mon host = $prefix.$i/g" ./ceph.conf
        sed -i "s/public network.*$/public network = $prefix.0\/24/g" ./ceph.conf
        if [[ `echo $?` != 0 ]]
        then
